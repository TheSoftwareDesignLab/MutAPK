	...
	public  void process(List<MutationLocation> locations, String extraPath, String apkName) throws IOException{
		...
		for (MutationLocation mutationLocation : locations) {
			try {
				...
				mutantRootFolder = getMutantsRootFolder()+File.separator+getAppName()+"-mutant"+mutantIndex + File.separator; 
			...
			results.add(executor.submit(new Callable<String>() {
				...
				public String call() {
					try {
						...
						//Set up folders
						String mutantRootFolder = getMutantsRootFolder()+File.separator+getAppName()+"-mutant"+currentMutationIndex + File.separator; 
